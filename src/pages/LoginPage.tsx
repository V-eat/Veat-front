...
export default function LoginPage() {
  ...
  const { signIn, signInWithOAuth, isAuthenticated, role, isLoading: authLoading } = useAuth();
  ...
  const handleOAuth = async (provider: 'google' | 'apple') => {
    ...
    try {
      ...
      // Supabase va rediriger vers le provider. Si popups bloquées ou erreur, on catch.
    } catch {
      setError('Impossible de lancer la connexion via ' + (provider === 'google' ? 'Google' : 'Apple'));
      setIsLoading(false);
    }
  };
        ...
        <div className="absolute inset-0 flex items-center justify-center p-12">
          <div className="text-center text-white">
            ...
            <p className="text-lg text-white/80 max-w-md">
              Commandez à l'avance et profitez de vos repas sans attente dans vos restaurants préférés.
            </p>
          </div>
        </div>
      </div>
